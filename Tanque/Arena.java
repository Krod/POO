...
public class Arena extends JComponent implements MouseListener, ActionListener, MouseMotionListener, KeyListener,
	...
	protected void paintComponent(Graphics g) {
		...
		if (System.currentTimeMillis() < temporizadorDoChat) {
			...
			// Uma lista auxilixar é utilizada para evitar que a thread faça alteração na
			// lista enquanto ela está sendo acessada;
			auxChat.clear();
			auxChat.addAll(Chat);
		...
		if (!comando.equals("cliente") && !comando.equals("servidor")) {
			...
			for (Tanque t : tanques) {
				...
				if (clicado) {
					...
					tanqueAtivo = t;
					/*
					 * switch(e.getButton()){ case MouseEvent.BUTTON1: t.aumentarVelocidade();
					 * break; case MouseEvent.BUTTON2: t.aumentarVelocidade(); break; case
					 * MouseEvent.BUTTON3: t.girarHorario(3); break; }
					 */
					break;
				}
			}
	...
	public static void main(String args[]) throws IOException {
		...
		comando = JOptionPane.showInputDialog("A máquina será cliente ou servidor?");
		comando = comando.toLowerCase();
		tanqueAtivo = new Tanque(100, 200, random.nextInt(360),
				new Color(random.nextInt(255), random.nextInt(255), random.nextInt(255)));
		tanqueAtivo.setEstaAtivo(true);

		String nome = JOptionPane.showInputDialog("Digite o nome do jogador.");
		tanqueAtivo.setNome(nome);

		arena.adicionaTanque(tanqueAtivo);
		if (comando.equals("cliente")) {
			System.out.println("Iniciando cliente");
			String ip = JOptionPane.showInputDialog("IP do servidor: ");
			porta = JOptionPane.showInputDialog("Porta do servidor: ");
			cliente = new Cliente(ip, Integer.valueOf(porta), nome);
			cliente.start();
		} else if (comando.equals("servidor")) {
			porta = JOptionPane.showInputDialog("Porta do servidor: ");
	...
	public static void adicionaMensagemNoChat(String s) {
		if (Arena.tanqueAtivo != null) {
			if (Arena.comando.equals("servidor")) {
				if (!s.isEmpty())
					Arena.Chat.addFirst(Arena.tanqueAtivo.getCor().getRGB() + "|" + Arena.tanqueAtivo.getNome()
							+ "(Servidor): " + s);
